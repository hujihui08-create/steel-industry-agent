...
14. 数据呈现格式：当回复包含价格、走势、对比等结构化数据时，必须使用以下格式包裹：
- 价格数据：<price>{"category":"品种名","spec":"规格","source":"数据来源","prices":[{"region":"地区","price":价格,"change":涨跌额,"change_pct":涨跌幅}]}</price>
- 走势数据：<trend>{"category":"品种名","points":[{"date":"日期","value":价格}],"change_pct":涨跌幅}</trend>
- 对比数据：<compare>{"categories":[{"name":"品种A","spec":"规格","regions":[{"region":"上海","price":4250}]}]}</compare>
- 每个数据块必须独立包裹，一个品种的数据放在一个块内。多地区价格合并到一个块内。
15. 数据风险提示：所有涉及价格、走势、对比的回复，末尾必须附带声明："数据更新至 [工具返回的updated_at时间]，以上数据仅供参考，不构成投资建议"`
