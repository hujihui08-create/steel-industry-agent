// ============================================================
// usePanelChat SSE 流式对话 Hook（工作台面板独立实例）
// 与 useChat 完全隔离，操作 usePanelChatStore 而非 useChatStore
// ============================================================
...
export function usePanelChat() {
  ...
  const sendMessage = useCallback(async (content: string) => {
    const store = usePanelChatStore.getState();
    ...
    const controller = chatApi.sendMessage(
      ...
      (chunk) => {
        const s = usePanelChatStore.getState();
      ...
      (status) => {
        usePanelChatStore.getState().setStatusMessage(status);
      },
      (eventType, data) => {
        const s = usePanelChatStore.getState();
        switch (eventType) {
          case 'plan_created':
            s.setAgentPlan(data as unknown as import('@/app/stores/panelChatStore').AgentPlan);
            break;
          case 'step_start': {
  ...
  const newSession = useCallback(() => {
    usePanelChatStore.getState().newSession();
  }, []);
  ...
  const loadSessions = useCallback(async () => {
    try {
      ...
      usePanelChatStore.getState().setSessions(sessions);
    } catch {
      // 静默失败
    }
  }, []);
